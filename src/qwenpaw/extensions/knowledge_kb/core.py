"""knowledge_kb core: business helpers (DB / extraction / chunking / query /
builtin packs / embedding-aware indexing). Originally extracted from the
standalone KB plugin's `server.py`; the HTTP layer (Handler class) lives in
`api/routes.py` as a FastAPI router instead.

LLM/embedding callsites route through the qwenpaw-aware bridges in
`bridges/`, so this module never reads DEEPSEEK_API_KEY or DASHSCOPE_API_KEY
from the env.
"""
...
from .bridges import embedding as embedding_provider
from .paths import (
    builtin_kb_dir as _builtin_kb_dir,
...
DB_PATH = _db_path()
BUILTIN_KB_DIR = _builtin_kb_dir()

MAX_UPLOAD_BYTES = 20 * 1024 * 1024
MAX_MANUAL_ENTRY_BYTES = 50 * 1024
ORPHAN_JOB_THRESHOLD_SECONDS = 600

# LLM/embedding availability is now driven by the knowledge agent's qwenpaw
# config (provider/model + embedding_config). No DEEPSEEK_API_KEY /
# DASHSCOPE_API_KEY env vars are read here.
EMBEDDING_BATCH_SIZE = 10
EMBEDDING_DEFAULT_PROVIDER = "qwenpaw-embedding"  # informational tag only
HYBRID_ALPHA = 0.5


def embedding_enabled() -> bool:
    """Return True iff the knowledge agent has a complete embedding config."""
    return embedding_provider.is_embedding_available()
...
def embedding_status() -> dict:
    ...
    return {
        ...
        "model": embedding_provider.current_model_name(),
    }
RAG_SYSTEM_PROMPT = (
...
# Directory creation is owned by manager.startup(); paths.data_dir() /
# paths.upload_dir() also lazy-mkdir on first access for safety.
...
def compute_unit_embeddings(ku_ids: list[str], batch_size: int = EMBEDDING_BATCH_SIZE) -> int:
    ...
    Silently no-op if embedding is unavailable. Logs and continues on batch
        ...
        try:
            for r, vec in zip(batch, vectors):
                ...
                conn.execute(
                    ...
                    (
                        blob,
                        str(embedding_provider.DEFAULT_DASHSCOPE_MODEL),
                        len(vec),
                        now_iso(),
                        r["id"],
                    ),
...
# `parse_uploaded_file()` from the legacy stdlib Handler was deleted; FastAPI's
# `UploadFile` in `api/routes.py::ingest_endpoint` handles multipart uploads.
