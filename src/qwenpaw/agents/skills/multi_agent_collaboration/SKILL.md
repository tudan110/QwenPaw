...
CMDB 拓扑查询、批量资源关系分析、需要生成 `echarts` 拓扑图的协作，也属于复杂任务。
...
- 需要并行执行的独立任务
- 查询一批 CI 之间的关系并生成拓扑图
- 结合导入结果范围做 CMDB 局部拓扑分析

**判断标准**：如果不确定任务会花多长时间，或者任务很复杂，优先使用后台模式。
特别是当目标 agent 还要继续调用其他 skill、查询 CMDB、生成图表代码时，不要使用实时模式长时间阻塞等待。
...
如果当前用户正在等待同一轮协作结果，查询状态时不要只查一次。
对于 CMDB 拓扑/关系分析这类任务，至少在合理间隔后继续查询 1-2 次，再决定是返回最终结果还是告诉用户任务仍在处理中。
