...
`deploy-all/qwenpaw/working/workspaces/query/skills/veops-cmdb`
...
- A2A 协作 `veops-cmdb` 与 `mysql-deadlock-inspector`
...
- `resource-discovery` / `topology-expansion` 先由 `veops-cmdb` 提供资源与关系证据
...
- `veops-cmdb` 继续保留在 query workspace，由新根因分析 skill 通过 A2A 协作使用
