...
const DIAGNOSIS_NUMBER_FIELDS: DiagnosisNumberField[] = [
  ...
  {
    key: "analysis_retry_max_attempts",
    label: "分析失败最大重试次数",
    group: "polling",
    min: 0,
    max: 10,
    step: 1,
    hint: "分析流、报告或卡片持久化失败后自动补偿的最大次数；达到上限后转人工处理。",
  },
  {
    key: "analysis_retry_base_delay_seconds",
    label: "分析失败首次重试延迟（秒）",
    group: "polling",
    min: 1,
    step: 10,
    hint: "失败补偿采用指数退避；后续等待时间按此值翻倍。",
  },
  {
    ...
    min: 60,
    step: 60,
    hint: "运行中的分析超过此时长会回收为待补偿处理，避免告警永久卡在分析中。",
  },
  {
