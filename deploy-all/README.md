...
一体化部署配置，包含 digital-workforce-portal 和 copaw 两个子应用。
...
| copaw | 8088 | 30088 | CoPaw 主后端 |

### 服务依赖关系

前端 `digital-workforce-portal` 通过 nginx 反向代理访问后端 `copaw` API：

- 前端请求 `/copaw-api/*` → nginx 代理 → `copaw:8088/*`
- 前端请求 `/portal-api/*` → nginx 代理 → `copaw:8088/api/portal/*`

说明：
- `/portal-api/*` 由 CoPAW 主进程通过自定义路由扩展提供
- 具体代码位于 `src/copaw/extensions/api/portal_backend.py`
- 外部系统接入逻辑位于 `src/copaw/extensions/integrations/`
...
helm install cnos-inoe-agent ./deploy-all/helm/cnos-inoe-agent \
  ...
  --set copaw.service.nodePort=32088
```
