...
qwenpaw cron create \
  ...
  --mode final \
  --text "请帮我巡检一下核心数据库 db_mysql_001。要求：1. 先协作 query 智能体使用 veops-cmdb 确认巡检对象的拓扑、资源名称、resId/CI ID 和 ciType。2. 如果存在多个候选资源，先明确列出候选项，不要默认任选一个。3. 一旦确认 resId 和 ciType，查询该资源类型的全部指标定义，提取全部指标编码。4. 调用指标数据接口，使用 resId + 全部指标编码数组完成巡检。5. 最后输出巡检结果、拓扑确认摘要、健康状态评估和指标数据表。"
```
